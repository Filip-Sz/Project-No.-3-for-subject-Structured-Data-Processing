...
BestTagsGaming <- GamingTags %>% filter(X._Count > 200) %>% select(X._TagName)

GamingTagsList <- list()
for (i in 1:nrow(BestTagsGaming)){
  GamingTagsList[i] <- BestTagsGaming[i,]
}
GamingTagsList <- head(GamingTagsList, 24)
...
tabela_podsumowanie_Sports <- data.frame(Pozycja = 1:24,
                                         wgViewCount = tabela_ViewCount[2],
...
colnames(tabela_podsumowanie_Sports) <- c("Pozycja", "Według ViewCount", "Według CommentCount", "Średnia CommentCount", "Według AnswerCount", "Średnia AnswerCount")
...
tabela_podsumowanie_Fitness <- data.frame(Pozycja = 1:24,
                                         wgViewCount = tabela_FitnessViewCount[2],
...
colnames(tabela_podsumowanie_Fitness) <- c("Pozycja", "Według ViewCount", "Według CommentCount", "Średnia CommentCount", "Według AnswerCount", "Średnia AnswerCount")
...
tabela_podsumowanie_Gaming <- data.frame(Pozycja = 1:24,
                                         wgViewCount = tabela_GamingViewCount[2],
...
colnames(tabela_podsumowanie_Gaming) <- c("Pozycja", "Według ViewCount", "Według CommentCount", "Średnia CommentCount", "Według AnswerCount", "Średnia AnswerCount")
